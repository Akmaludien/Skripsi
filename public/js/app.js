...
function initTelegramButton() {
    ...
    style.textContent = `
        @keyframes tg-pulse {
            0% { box-shadow: 0 6px 20px rgba(0,136,204,0.4); }
            50% { box-shadow: 0 6px 20px rgba(0,136,204,0.7), 0 0 0 8px rgba(0,136,204,0.1); }
            100% { box-shadow: 0 6px 20px rgba(0,136,204,0.4); }
        }
        #tg-btn { animation: tg-pulse 2.5s infinite; }
        #tg-btn:hover { animation: none; transform: scale(1.05); box-shadow: 0 8px 28px rgba(0,136,204,0.6) !important; }
    `;
    ...
    btn.style.cssText = 'position:fixed; bottom:24px; right:24px; display:flex; align-items:center; gap:12px; background:linear-gradient(135deg, #0088cc, #005f8f); color:white; padding:16px 24px; border-radius:60px; text-decoration:none; box-shadow:0 6px 20px rgba(0,136,204,0.4); font-family:\"Segoe UI\",sans-serif; font-size:15px; font-weight:600; z-index:9999; transition:all 0.3s ease;';
    btn.innerHTML = `
